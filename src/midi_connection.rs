...
pub fn getOutput (port_name: &str) -> Result<MidiOutputConnection, ConnectError<MidiOutput>> {
    let output = MidiOutput::new(appName).unwrap();
    let port_number = match get_output_port_index(&output, port_name) {
        Err(error) => return Err(ConnectError::new(ConnectErrorKind::Other("No output port with specified name"), output)),
        Ok(value) => value
    };
    output.connect(port_number, port_name)
}

pub fn getInput<F, T: Send> (port_name: &str, callback: F, data: T) -> Result<MidiInputConnection<T>, ConnectError<MidiInput>> 
where F: FnMut(u64, &[u8], &mut T) + Send + 'static {
    let input = MidiInput::new(appName).unwrap();
    let port_number = match get_input_port_index(&input, port_name) {
        Err(error) => return Err(ConnectError::new(ConnectErrorKind::Other("No input port with specified name"), input)),
        Ok(value) => value
    };
