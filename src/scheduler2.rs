...
impl Iterator for Scheduler {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if ticked {
            ...
            println!("TICK {} - {}", from.as_float(), to.as_float());
        } else {
            println!("SUB TICK {} - {}", from.as_float(), to.as_float());
        }

        let this_time = Instant::now();
        let duration = this_time.duration_since(self.last_time).as_millis();
        if duration >= 9 {
            println!("DELAY {:?}", duration);
        }
